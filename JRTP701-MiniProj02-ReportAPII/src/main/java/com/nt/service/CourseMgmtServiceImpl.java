...
public class CourseMgmtServiceImpl implements ICourseMgmtService {
	...
	public void generatePdfReportAll(HttpServletResponse res) throws Exception {
		//To get all course results from DB Table
				List<CourseDetails> list=courseRepo.findAll();
				//To copy List<CourseDetails> into the List<SearchResults>
				List<SearchResults> listResults=new ArrayList<SearchResults>();
				//To print the List use farEach method
				list.forEach(course->{
					//define the object where the data should be saved
					SearchResults result=new SearchResults();
					//copy the rusultent output data into outer variable
					BeanUtils.copyProperties(course, result);
					//paste that data into the object where rows should be saved
					listResults.add(result);	
				});
				Document document=new Document(PageSize.A4);
				PdfWriter.getInstance(document, res.getOutputStream());
	...
	public void generateXlsReportAll(HttpServletResponse res) throws Exception {
		...
		HSSFWorkbook workbook=new HSSFWorkbook();
		HSSFSheet sheet1=workbook.createSheet("CourseDetails");
		// Create font for the header row (Bold & Italic)
